...
export default function SongPrompts() {
  ...
  const fetchPromptData = async () => {
    setLoading(true);
    try {
      const response = await api.get('/api/prompts/song_master');
  ...
  const handleSave = async () => {
    ...
    try {
      await api.patch('/api/prompts/song_master', {
        ...
        <div className="mt-6 px-6">
          ...
          <div className="">
            <div className="flex items-start justify-between mb-6">
              ...
              <button className="flex items-center gap-2 mt-4  bg-gradient-to-r from-[#9458E8] via-[#A43EE7] to-[#CA00E5] text-white px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90">
