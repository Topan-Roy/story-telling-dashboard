import { type SinglePckageDataType } from "../data/packageData";


export default function TrComponentUpdate(
    { packageData, setShowUpdate }: { packageData: SinglePckageDataType, setShowUpdate: React.Dispatch<React.SetStateAction<boolean>> }
) {
    
    return (
        <>
            <div className=' p-6 w-full absolute left-0 top-14 h-[94vh] flex items-center justify-center' style={{background: "rgb(202, 213, 226, 0.5)"}}> 
                <div className='bg-white p-6 rounded-[8px]'>
                    <hr className='h-[1px] bg-[#E5E7EB] text-[#E5E7EB]' />
                    <p className='font-[500] text-[13.6px] leading-6 text-[#374151] inter-font my-[17px]'>Update Package</p>
                    <form action="" method='POST'>
                        <div className='flex items-center justify-start gap-4 md:flex-nowrap flex-wrap'>
                            <div className='w-full'>
                                ...
                                <input type="text" defaultValue={packageData.Package} className='my-[5px] rounded-[6px] border-[1px] border-[#D1D5DB] py-[7px] pl-[13px] w-full outline-none' name="package-name" id="package-name" />
                            </div>
                            <div className='w-full'>
                                <label className='font-[500] text-[11.9px] leading-5 inter-font' htmlFor="coin">Coin</label>
                                <input type="text" defaultValue={packageData.Coins} className='my-[5px] rounded-[6px] border-[1px] border-[#D1D5DB] py-[7px] pl-[13px] w-full outline-none' name="coin" id="coin" />
                            </div>
                            <div className='w-full'>
                                <label className='font-[500] text-[11.9px] leading-5 inter-font' htmlFor="price">Price (£)</label>
                                <input type="text" defaultValue={packageData.Price} className='my-[5px] rounded-[6px] border-[1px] border-[#D1D5DB] py-[7px] pl-[13px] w-full outline-none' name="price" id="price" />
                            </div>
                        </div>
